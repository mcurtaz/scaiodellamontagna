...
- These are linked via **itinerari correlati**, qualified with a relation type (`prosecuzione` / `variante` / `nella_zona`) so the frontend can distinguish "si collega con..." / "varianti" / "itinerari nella stessa zona". In Directus this is a **single self-referencing M2M field** on `itinerari` (junction collection `itinerari_correlati` with an extra `tipo` field), not a plain M2M relation.
- The relation is modeled as **symmetric, with no "da/a" direction in the schema**: `prosecuzione` and `variante` describe how two routes relate to each other, not which one comes first, and `nella_zona` has no direction at all. Editors add a related route from either side using the same field, the same way, regardless of `tipo`. Linking A and B once is enough — the Astro build queries the junction in both directions so the relation shows up on both routes' pages automatically. (If a directional phrasing for `prosecuzione` — "puoi proseguire fino a..." vs "arrivi qui da..." — is wanted later, it can be added as an optional extra field without changing this structure.)
